...
### Temp to decrease over during the model
...
outputPath = f'./output/isotropic_decreasingT_Tstart={Temp_start}_Tend={Temp_end}_duration={model_duration}_csize={csize}_Diffusion-Decay_test/'
...
while model_time < total_time:
    ...
    dt = min( (0.5 * (mesh.get_min_radius()**2 / D_val) ) , nd(50*u.megayear) )
...
plt.savefig(f'{outputPath}isotropic_diffusion-decay-U_Pb_profiles-Tstart={Temp_start}_Tend={Temp_end}-C_{model_duration}Myr_csize={csize}.pdf')
