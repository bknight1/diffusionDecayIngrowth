...
outputDir = f'./output/CZGM_comp_model_annulus_cartesian_{dt_fac}dt-update/'
...
n = 4
plt.scatter(CZGM_PTt_path.iloc[:,1][::n]-273.15, CZGM_PTt_path.iloc[:,0][::n]/1e4, marker='x', c='k')

for i in CZGM_PTt_path.iloc[:,1][::n].index:
    plt.text(CZGM_PTt_path.iloc[:,1][i]-273.15, CZGM_PTt_path.iloc[:,0][i]/1e4, round(CZGM_PTt_path.iloc[:,2][i], 2) )

plt.xlabel('T [$\degree$C]')
plt.ylabel('P [GPa]')
plt.grid(ls= ':', alpha=0.5)
plt.savefig(f'{outputDir}PT_path.pdf')
...
plt.savefig(f'{outputDir}em_frac_initial_profile.pdf')
...
def update_domain(step, shell_radius):
    ...
    r = sp.sqrt( mesh.X[0]**2 + mesh.X[1]**2 )
...
# %%
def solve_diff_eq(solver, u, u_star, flux_vector, flux_vector_star, dt, spherical=False):
    ### setup f0 term
    ...
    ### setup f1 term with optional spherical correction
    theta = 0
    diffusion_CM = uw.constitutive_models.DiffusionModel
    
    if spherical:
        # Add spherical geometric term: (2/r) * D * dC/dr for 3D spherical symmetry
        r = sp.sqrt(mesh.X[0]**2 + mesh.X[1]**2)
        r_safe = sp.Max(r, 1e-10)  # Avoid division by zero at center
        
        # Spherical correction adds (2/r) * flux_radial to account for r² geometry
        # In 2D we only have cylindrical (r), so we add extra factor to mimic r²
        correction_strength = 0.2
        flux_correction = correction_strength * (1.0 / r_safe) * flux_vector
        diffusion_CM.flux = flux_vector + flux_correction
        flux_term = flux_vector + flux_correction
    else:
        diffusion_CM.flux = flux_vector
        flux_term = flux_vector

    solver.constitutive_model =  diffusion_CM

    solver.flux = theta*flux_term + ((1-theta)*flux_vector_star)
...
while model_time < nd(time_evolution[-1]*u.megayear):
    ...
    while model_time < nd(time_evolution[step]*u.megayear):
        ...
        ### solve the equation
        solve_diff_eq(diff_Fe, Fe, Fe_star, Fe_flux_term, Fe_flux_star, dt, spherical=True)
        solve_diff_eq(diff_Mg, Mg, Mg_star, Mg_flux_term, Mg_flux_star, dt, spherical=True)
        solve_diff_eq(diff_Mn, Mn, Mn_star, Mn_flux_term, Mn_flux_star, dt, spherical=True)
...
profile_coords = np.zeros((Rr_nd.shape[0],2))
#### profile along garnet from core to rim along x axis, matching the mesh nodal points
profile_coords[:,0] = np.linspace(0., Rr_nd.max(), Rr_nd.shape[0])
profile_coords[:,1] = 0.
...
plt.plot(profile_coords[:,0]*Rr.max(), Fe_profile_Final, c='red', label='Fe')
plt.plot(profile_coords[:,0]*Rr.max(), Mg_profile_Final, c='green', label='Mg')
plt.plot(profile_coords[:,0]*Rr.max(), Mn_profile_Final, c='blue', label='Mn')
plt.plot(profile_coords[:,0]*Rr.max(), Ca_profile_Final, c='gold', label='Ca')
...
plt.savefig(f'{outputDir}final_TS_CZGM_comp_annulus.pdf')
